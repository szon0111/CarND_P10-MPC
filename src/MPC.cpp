...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int delta_w = 5;
    const int a_w = 5;
    const int delta_change_w = 50;
    const int a_change_w = 5;

    // cost for CTE, psi error, velocity
    for (int t = 0; t < N; t++) {
      fg[0] += cte_w* CppAD::pow(vars[cte_start + t], 2);
      fg[0] += epsi_w * CppAD::pow(vars[epsi_start + t], 2);
      fg[0] += v_w * CppAD::pow(vars[v_start + t] - MAX_V, 2);
    }
  ...
  // cost for change in steering and acceleration
    for (int t = 0; t < N - 2; t++) {
      fg[0] += delta_change_w * pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += a_change_w * pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
    
    // adjust index values for starting index
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set current values to the limits
